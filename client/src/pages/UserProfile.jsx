...
// ── Portfolio gallery + lightbox (shared with ViewProfile) ───────────────────
          ...
          {items.map((item, ii) => {
            ...
            return (
              <div key={ii} style={ps.projectCard}>
                <div style={ps.coverWrap} onClick={() => images.length && setLightbox({ itemIdx: ii, imgIdx: 0 })}>
                  {cover
                    ? <img src={cover.url} alt={item.title} style={ps.coverImg} />
                    : <div style={ps.coverPlaceholder}><span style={{ fontSize: 36 }}>🖼</span><span style={{ fontSize: 12, color: '#9ca3af', marginTop: 6 }}>No images</span></div>
                  }
                  {images.length > 1 && <div style={ps.imageBadge}>🖼 {images.length}</div>}
                  {images.length > 0 && <div style={ps.coverOverlay}><span style={ps.viewLabel}>View gallery</span></div>}
                </div>
                {images.length > 1 && (
                  <div style={ps.thumbStrip}>
                    {images.slice(0, 4).map((img, ji) => (
                      <div key={ji} style={ps.thumbWrap} onClick={() => setLightbox({ itemIdx: ii, imgIdx: ji })}>
                        <img src={img.url} alt="" style={ps.thumb} />
                        {ji === 3 && images.length > 4 && <div style={ps.thumbMore}>+{images.length - 4}</div>}
                      </div>
                    ))}
                  </div>
                )}
                <div style={ps.info}>
                  <div style={ps.titleRow}>
                    ...
                    {item.url && <a href={item.url} target="_blank" rel="noreferrer" style={ps.linkBtn}>🔗 Visit</a>}
                  </div>
            ...
            <div style={ps.lightbox} onClick={e => e.stopPropagation()}>
              ...
              <div style={ps.caption}>
                <div style={ps.captionTitle}>
                  {item.title}
                  {item.url && <a href={item.url} target="_blank" rel="noreferrer" style={ps.captionLink}>🔗 Visit project</a>}
                </div>
